...
class WordScoreService
{
    ...
    {
        //try to get word score from database
        $score = WordScore::where('word', $request->input('word'))->first();

        //if score for this word doesn't exist in database, get it from external data source and save to database
        if ( ! $score ) {
            ...
            //calculate score based on positive/negative count
            $score = $this->calculateScore($positive, $negative);

            //create record in our database to make future requests faster
    ...
    {
        //if positive and negative equals zero, then score is zero. Also prevent 'division by zero' exceptions in following calculations
